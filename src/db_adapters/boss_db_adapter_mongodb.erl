...
execute({WriteMode, ReadMode, {rsc_read_conn, RSConn, Connection}, Database}, Fun) ->
    case mongo:do(WriteMode, ReadMode, Connection, Database, Fun) of
        {failure, {connection_failure, _, closed}} ->
            ...
            {reconnected_read, mongo:do(WriteMode, ReadMode, NewConnection, Database, Fun),  {WriteMode, ReadMode, {rsc_read_conn, RSConn, NewConnection}, Database}};
        Res ->
            Res
    end;
execute({WriteMode, ReadMode, {rsc_write_conn, RSConn, Connection}, Database}, Fun) ->
    case mongo:do(WriteMode, ReadMode, Connection, Database, Fun) of
        {failure, {connection_failure, _, closed}} ->
            case mongo_replset:primary(RSConn) of
                {ok, NewConnection} ->
                    {reconnected_write, mongo:do(WriteMode, ReadMode, NewConnection, Database, Fun), {WriteMode, ReadMode, {rsc_write_conn, RSConn, NewConnection}, Database}};
                Error = {error, _} ->
                    Error
            end;
        Res ->
...
execute({WriteMode, ReadMode, {rsc_read_conn, RSConn, Connection}, Database, User, Password}, Fun) ->
    ...
    case mongo:do(WriteMode, ReadMode, Connection, Database, AuthFun) of
        {failure, {connection_failure, _, closed}} ->
            NewConnection = read_connect1(ReadMode, RSConn),
            {reconnected_read, mongo:do(WriteMode, ReadMode, NewConnection, Database, AuthFun), {WriteMode, ReadMode, {rsc_read_conn, RSConn, NewConnection}, Database, User, Password}};
        Res ->
            Res
    end;
execute({WriteMode, ReadMode, {rsc_write_conn, RSConn, Connection}, Database, User, Password}, Fun) ->
    ...
    case mongo:do(WriteMode, ReadMode, Connection, Database, AuthFun) of
        {failure, {connection_failure, _, closed}} ->
            case mongo_replset:primary(RSConn) of
                {ok, NewConnection} ->
                    {reconnected_write, mongo:do(WriteMode, ReadMode, NewConnection, Database, AuthFun), {WriteMode, ReadMode, {rsc_write_conn, RSConn, NewConnection}, Database, User, Password}};
                Error = {error, _} ->
                    Error
            end;
        Res ->
